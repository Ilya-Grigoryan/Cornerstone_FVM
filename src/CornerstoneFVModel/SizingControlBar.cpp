...
void CSizingControlBar::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
    ...
    if (m_pDockBar != NULL)
    {
		...
		pParent->ModifyStyle(0x00003B00|WS_THICKFRAME, 0x00003300);
	
		pParent->MoveWindow(300,100,200,100);
    }
    else
    {
...
void	CSizingControlBar::SetSize(CSize size,UINT nDockBarID)
{
    ...
    m_sizeVert = size;
    m_sizeFloat = size;
	pMainFrm -> DockControlBar(this, nDockBarID);
	m_pDockSite->DelayRecalcLayout();
}
